...
import datetime
import time
from ab_birnn2 import RNN
import data_process as data_helpers
...
# Data loading params
tf.flags.DEFINE_string("train_dir", "ABLSTM_data/training/TRAIN_FILE.TXT", "Path of train data")
...
tf.flags.DEFINE_integer("big_num", 50, "Big number  (Default: 50)")
# tf.flags.DEFINE_float("dropout_keep_prob", 0.5, "Dropout keep probability (Default: 0.5)")
# tf.flags.DEFINE_float("l2_reg_lambda", 3.0, "L2 regularization lambda (Default: 3.0)")
...
def train():
    ...
    print("x dev before={0}".format(x_dev.shape))
    ...
    print("x dev={0}".format(x_dev.shape))
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for batch in batches:
                ...
                # Train
                total_shape = []
                total_num = len(x_batch[0])
                total_shape.append(total_num)
                total_shape = np.array(total_shape)
                #print("total shape", x_batch[0])
                feed_dict = {
                    rnn.input_text: x_batch[0],
                    rnn.input_pos1: x_batch[1],
                    rnn.input_pos2: x_batch[2],
                    rnn.total_shape: total_shape,
                    rnn.input_y: y_batch,
                ...
                if step % FLAGS.evaluate_every == 0:
                    print("\nEvaluation:")
                    feed_dict = {
                        rnn.input_text: x_dev[0],
                        rnn.input_pos1: x_dev[1],
                        rnn.input_pos2: x_dev[2],
                        rnn.input_y: y_dev,
                        rnn.dropout_keep_prob: 1.0
                    }
                    summaries, loss, accuracy, predictions = sess.run(
                        [dev_summary_op, rnn.loss, rnn.accuracy, rnn.predictions], feed_dict)
